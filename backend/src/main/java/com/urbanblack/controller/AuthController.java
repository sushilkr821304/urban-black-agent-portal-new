...
public class AuthController {
    ...
    public ResponseEntity<?> sendOtp(@RequestBody java.util.Map<String, String> request) {
        ...
        if (phoneNumber == null || phoneNumber.length() != 10) {
            return ResponseEntity.badRequest()
                    .body(java.util.Collections.singletonMap("message", "Invalid phone number"));
        }

        if (userRepository.findByPhoneNumber(phoneNumber).isPresent()) {
            return ResponseEntity.badRequest()
                    .body(java.util.Collections.singletonMap("message", "Phone number is already registered"));
        }
    ...
    public ResponseEntity<?> changePassword(@RequestBody java.util.Map<String, String> request,
        ...
        if (!encoder.matches(currentPassword, user.getPassword())) {
            return ResponseEntity.badRequest()
                    .body(java.util.Collections.singletonMap("message", "Incorrect current password"));
        }
